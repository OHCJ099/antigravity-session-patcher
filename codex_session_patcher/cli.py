...
def resolve_session_format(args) -> SessionFormat:
    ...
    if fmt == 'codex':
        ...
        # 本 fork 面向 Antigravity Desktop：auto 优先选择本机会话，其次保持上游兼容。
        antigravity_dir = os.path.join(os.path.expanduser("~"), ".gemini", "antigravity", "brain")
...
def main():
    parser = argparse.ArgumentParser(
        description='Session Patcher - 清理 Antigravity Desktop / Codex CLI / Claude Code / OpenCode 会话中的拒绝回复和无效 thinking blocks'
    )

    # 会话清理参数
    parser.add_argument('--session-dir', default=None,
                        help='会话目录 (默认根据 --format 自动选择)')
    parser.add_argument('--format', choices=['antigravity', 'codex', 'claude-code', 'opencode', 'auto'],
                        default='auto',
    ...
    if session_format == SessionFormat.OPENCODE:
        _cli_process_opencode(args, mock_response, custom_keywords, clean_reasoning)
        return
    ...
    format_label = {
        ...
        SessionFormat.ANTIGRAVITY: 'Antigravity Desktop',
    }.get(session_format, session_format.value)
...
def _cli_process_opencode(args, mock_response: str, custom_keywords, clean_reasoning: bool):
