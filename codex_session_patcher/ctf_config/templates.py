...
# ─── Antigravity Desktop 专用模板 ──────────────────────────────────────────────
ANTIGRAVITY_CTF_OPTIMIZED = _load_prompt("antigravity_ctf_optimized.md")


def adapt_codex_prompt_to_antigravity(prompt: str) -> str:
    """将历史 Codex CTF 提示词转换为 Antigravity Desktop/CLI 语境。

    这里用于两处：
    1. 内置模板：复用上游 Codex 的简版/通用模板，但展示为 Antigravity 模板；
    2. 兼容迁移：用户过去保存到 codex 模板槽里的自定义模板，在 Antigravity UI 中可继续看到。
    """
    replacements = {
        "Codex CLI": "Antigravity Desktop / Antigravity CLI",
        "Codex": "Antigravity",
        "codex -p ctf": "antigravity.exe chat",
        "~/.codex": "%APPDATA%/antigravity",
    }
    result = prompt
...
ANTIGRAVITY_CTF_PRIVATE_DEPLOY = adapt_codex_prompt_to_antigravity(SECURITY_MODE_PROMPT)
ANTIGRAVITY_CTF_GENERAL = adapt_codex_prompt_to_antigravity(SECURITY_MODE_PROMPT_GENERAL)
...
BUILTIN_TEMPLATES: dict = {
    'antigravity': [
        {'name': 'Optimized CTF Workflow', 'prompt': ANTIGRAVITY_CTF_OPTIMIZED, 'file': 'ctf_optimized.md', 'default': True},
        {'name': 'CTF Private Deploy', 'prompt': ANTIGRAVITY_CTF_PRIVATE_DEPLOY, 'file': 'ctf_private_deploy.md'},
        {'name': 'General Security Testing', 'prompt': ANTIGRAVITY_CTF_GENERAL, 'file': 'ctf_general.md'},
    ],
    'codex': [
