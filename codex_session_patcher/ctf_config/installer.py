...
from .templates import (
    ...
    ANTIGRAVITY_CTF_OPTIMIZED,
    BUILTIN_TEMPLATES,
)
from .status import (
    check_ctf_status, CTFStatus, GLOBAL_MARKER, CTF_MARKER, ANTIGRAVITY_MARKER,
    DEFAULT_CLAUDE_CTF_WORKSPACE, DEFAULT_OPENCODE_CTF_WORKSPACE,
    expand_user_path, default_antigravity_dir, default_antigravity_global_config,
    default_antigravity_prompt_path, default_antigravity_profile_workspace,
    detect_antigravity_profile_workspace, detect_antigravity_global_workspaces,
    antigravity_profile_prompt_path_for_workspace, antigravity_global_prompt_path_for_workspace,
)
...
class CTFConfigInstaller:
    ...
    def __init__(self):
        self.codex_dir = expand_user_path("~/.codex")
...
class AntigravityCTFInstaller:
    """Antigravity Desktop CTF 配置安装器。

    Antigravity 的规则机制是项目级 Markdown rule：
    `.agent/rules/*.md`，通过 frontmatter `trigger: always_on` 自动加载。

    - Profile 模式：写入最近/当前 Antigravity 项目的 `.agent/rules/ctf_optimized.md`。
    - 全局模式：没有真正全局 TOML；本工具把同一 always_on rule 镜像到所有已知
      Antigravity 工作区，并在 `~/.gemini/antigravity/prompts/ctf_global_manifest.json`
      记录受管理路径，便于更新和卸载。
    """
    ...
    def __init__(self, workspace_dir: str = None):
        self.antigravity_dir = default_antigravity_dir()
        self.global_config_path = default_antigravity_global_config()
        self.global_prompts_dir = os.path.dirname(self.global_config_path)
        self.global_prompt_path = default_antigravity_prompt_path()
        self.detected_workspace_dir = workspace_dir or detect_antigravity_profile_workspace()
        self.workspace_dir = self.detected_workspace_dir or default_antigravity_profile_workspace()
        self.profile_config_path = antigravity_profile_prompt_path_for_workspace(self.workspace_dir)
        self.profile_prompt_path = self.profile_config_path
    ...
    def _get_prompt_content(self) -> str:
        try:
            ...
            pass
        for tpl in BUILTIN_TEMPLATES.get('antigravity', []):
            if tpl.get('default'):
                return tpl['prompt']
        return ANTIGRAVITY_CTF_OPTIMIZED

    @staticmethod
    def _rule_content(prompt: str, label: str) -> str:
        label_text = "profile" if label == "profile" else "global"
        return "\n".join([
            "---",
            "trigger: always_on",
            f"description: CTF/pentest sandbox instructions managed by antigravity-session-patcher ({label_text})",
            "---",
            "",
            f"{ANTIGRAVITY_MARKER} Antigravity CTF {label_text} mode managed by antigravity-session-patcher",
            "",
            prompt.rstrip(),
            "",
        ])

    def _write_rule(self, path: str, prompt: str, label: str) -> Optional[str]:
        """写入受管理 rule；如存在非本工具文件，先备份。"""
        ...
        if os.path.exists(path):
            try:
                ...
                existing = ""
            if ANTIGRAVITY_MARKER not in existing:
                backup_path = self._backup_config(path)
        with open(path, "w", encoding="utf-8") as f:
            f.write(self._rule_content(prompt, label))
        return backup_path

    @staticmethod
    def _remove_rule_if_managed(path: str, label: Optional[str] = None) -> bool:
        ...
        if ANTIGRAVITY_MARKER not in content:
            return False
        if label and f"{label} mode" not in content:
            return False
        os.remove(path)
        return True

    @staticmethod
    def _cleanup_empty_rule_dirs(rule_path: str):
        rules_dir = os.path.dirname(rule_path)
        agent_dir = os.path.dirname(rules_dir)
        for path in [rules_dir, agent_dir]:
            try:
    ...
    def install(self, custom_prompt: str = None, injection_mode: str = "always_on_rule") -> tuple[bool, str]:
        """安装 Antigravity Profile CTF 模式。"""
        try:
            if not self.detected_workspace_dir and not os.path.isdir(self.workspace_dir):
                return False, (
                    "Antigravity Profile 模式安装失败: 未检测到当前 Antigravity 项目工作区。"
                    "请先在 Antigravity Desktop 打开目标项目/新建一次对话，然后再点击启用。"
                )
            prompt = custom_prompt or self._get_prompt_content()
            backup_path = self._write_rule(self.profile_prompt_path, prompt, "profile")
            details = [
                f"✓ 已启用当前 Antigravity 工作区 Profile 模式: {self.workspace_dir}",
                f"✓ 已写入项目 always_on rule: {self.profile_prompt_path}",
                "✓ 注入方式: .agent/rules Markdown rule（trigger: always_on）",
                "无需独立启动脚本；在该项目中新建对话即可生效，若已有窗口未刷新请重启 Antigravity Desktop。",
            ]
            if backup_path:
                details.insert(2, f"✓ 已备份原 rule 到: {backup_path}")
            return True, "\n".join(details)
        except Exception as e:
            return False, f"Antigravity Profile 模式安装失败: {e}"

    def uninstall(self) -> tuple[bool, str]:
        try:
            removed = []
            if self._remove_rule_if_managed(self.profile_prompt_path, "profile"):
                removed.append(self.profile_prompt_path)
                self._cleanup_empty_rule_dirs(self.profile_prompt_path)
            if not removed:
                return True, "Antigravity Profile 模式未安装"
            return True, "已卸载 Antigravity Profile 模式:\n" + "\n".join(f"✓ {p}" for p in removed)
        except Exception as e:
            return False, f"Antigravity Profile 模式卸载失败: {e}"

    def _global_workspaces(self) -> list[str]:
        workspaces = detect_antigravity_global_workspaces()
        if self.detected_workspace_dir and os.path.isdir(self.detected_workspace_dir):
            workspaces.insert(0, self.detected_workspace_dir)
        return list(dict.fromkeys(os.path.normpath(p) for p in workspaces if os.path.isdir(p)))

    def install_global(self, injection_mode: str = "always_on_rule", custom_prompt: str = None) -> tuple[bool, str]:
        """安装 Antigravity 全局 CTF 模式（镜像到所有已知工作区）。"""
        try:
            prompt = custom_prompt or self._get_prompt_content()
            os.makedirs(self.global_prompts_dir, exist_ok=True)
            with open(self.global_prompt_path, "w", encoding="utf-8") as f:
                f.write(prompt)

            workspaces = self._global_workspaces()
            if not workspaces:
                return False, (
                    "Antigravity 全局模式安装失败: 未检测到任何 Antigravity 工作区。"
                    "请先用 Antigravity 打开至少一个项目，或创建 G:\\测试工作区\\Antigravity_workspace。"
                )

            rule_paths = []
            backups = []
            for workspace in workspaces:
                rule_path = antigravity_global_prompt_path_for_workspace(workspace)
                backup = self._write_rule(rule_path, prompt, "global")
                if backup:
                    backups.append(backup)
                rule_paths.append(rule_path)

            manifest = {
                "managed_by": "antigravity-session-patcher",
                "marker": ANTIGRAVITY_MARKER,
                "mode": "global",
                "injection_mode": "always_on_rule",
                "prompt_path": self.global_prompt_path,
            ...
            details = [
                "✓ 已启用 Antigravity 全局 CTF 模式",
                f"✓ 已写入提示词源文件: {self.global_prompt_path}",
                f"✓ 已写入 manifest: {self.global_config_path}",
                f"✓ 已同步 always_on rule 到 {len(rule_paths)} 个已知工作区",
                "⚠ Antigravity 无单一全局 system_prompt_file；新建/切换工作区后可再次点击启用全局以补写 rule。",
            ]
            if backups:
                details.append(f"✓ 已备份 {len(backups)} 个原 rule 文件")
            return True, "\n".join(details)
        except Exception as e:
            return False, f"Antigravity 全局模式安装失败: {e}"
    ...
    def uninstall_global(self) -> tuple[bool, str]:
        try:
            ...
            if os.path.exists(self.global_config_path):
                try:
                    manifest = json.load(open(self.global_config_path, "r", encoding="utf-8"))
                    rule_paths.extend(manifest.get("rule_paths", []))
                except Exception:
                    pass
            for workspace in self._global_workspaces():
                rule_paths.append(antigravity_global_prompt_path_for_workspace(workspace))

            removed = []
            for rule_path in list(dict.fromkeys(rule_paths)):
                if self._remove_rule_if_managed(rule_path, "global"):
                    removed.append(rule_path)
                    self._cleanup_empty_rule_dirs(rule_path)
            if os.path.exists(self.global_config_path):
                os.remove(self.global_config_path)
                removed.append(self.global_config_path)

            if not removed:
                return True, "Antigravity 全局模式未安装"
            return True, "已卸载 Antigravity 全局模式:\n" + "\n".join(f"✓ {p}" for p in removed)
        except Exception as e:
            return False, f"Antigravity 全局模式卸载失败: {e}"

    def update_profile_prompt(self, prompt: str) -> bool:
        if not os.path.exists(self.profile_prompt_path):
            return False
        self._write_rule(self.profile_prompt_path, prompt, "profile")
        return True

    def update_global_prompt(self, prompt: str) -> bool:
        status = check_ctf_status()
        if not status.antigravity_global_installed:
            return False
        os.makedirs(self.global_prompts_dir, exist_ok=True)
        with open(self.global_prompt_path, "w", encoding="utf-8") as f:
            f.write(prompt)
        ...
        for workspace in self._global_workspaces():
            rule_paths.append(antigravity_global_prompt_path_for_workspace(workspace))
        for rule_path in list(dict.fromkeys(rule_paths)):
            self._write_rule(rule_path, prompt, "global")
        return True
...
class ClaudeCodeCTFInstaller:
    ...
    def __init__(self):
        ...
        self.settings_local = expand_user_path("~/.claude/settings.local.json")
