...
# ─── Antigravity Desktop 策略 ─────────────────────────────────────────────────

class AntigravityFormatStrategy(FormatStrategy):
    """Antigravity Desktop JSONL 格式。

    Antigravity 的真实会话日志位于：
    ~/.gemini/antigravity/brain/<trajectory-id>/.system_generated/logs/transcript.jsonl
    ...
    def get_assistant_messages(self, lines: List[Dict[str, Any]]) -> List[Tuple[int, Dict[str, Any]]]:
        ...
        for idx, line in enumerate(lines):
            if (
                line.get('source') == 'MODEL'
                and line.get('type') == 'PLANNER_RESPONSE'
                and line.get('content') is not None
            ):
                messages.append((idx, line))
        return messages

    def get_thinking_items(self, lines: List[Dict[str, Any]]) -> List[Tuple[int, Dict[str, Any]]]:
        # Antigravity 的 thinking/reasoning 嵌入在消息对象里，不是独立行。
        return []
        ...
        if isinstance(content, list):
            ...
            for item in content:
                if isinstance(item, dict):
                    text = item.get('text') or item.get('content') or item.get('input_text') or ''
                    if text:
...
def get_format_strategy(fmt: SessionFormat) -> FormatStrategy:
    if fmt == SessionFormat.CODEX:
        ...
        return AntigravityFormatStrategy()
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            for i, raw_line in enumerate(f):
                ...
                if source in ('MODEL', 'USER_EXPLICIT', 'SYSTEM') and line_type in (
                    'PLANNER_RESPONSE', 'USER_INPUT', 'SYSTEM_MESSAGE',
                    'ERROR_MESSAGE', 'CONVERSATION_HISTORY', 'EPHEMERAL_MESSAGE',
                    'CHECKPOINT',
                ):
                ...
                if data.get('role') in ('system', 'user', 'assistant', 'tool') and 'content' in data:
                    return SessionFormat.ANTIGRAVITY
    except Exception:
...
def _detect_format_from_path(file_path: str) -> SessionFormat:
    ...
    if expanded.startswith(antigravity_dir):
        return SessionFormat.ANTIGRAVITY
