...
# 默认路径
DEFAULT_SESSION_DIR = expand_user_path("~/.codex/sessions/")
DEFAULT_CLAUDE_SESSION_DIR = expand_user_path("~/.claude/projects/")
DEFAULT_ANTIGRAVITY_SESSION_DIR = os.path.join(expand_user_path("~"), ".gemini", "antigravity", "brain")
DEFAULT_MEMORY_FILE = expand_user_path("~/.codex/memories/MEMORY.md")
...
def _resolve_format(format_str: str) -> Optional[SessionFormat]:
    ...
    elif format_str == 'antigravity':
        return SessionFormat.ANTIGRAVITY
    return None  # auto
...
def _to_schema_format(fmt: SessionFormat) -> SessionFormatEnum:
    ...
    elif fmt == SessionFormat.ANTIGRAVITY:
        return SessionFormatEnum.ANTIGRAVITY
...
def list_sessions(
    ...
    if session_format is None:
        ...
        if os.path.exists(DEFAULT_ANTIGRAVITY_SESSION_DIR):
            scan_targets.append((DEFAULT_ANTIGRAVITY_SESSION_DIR, SessionFormat.ANTIGRAVITY))
        ...
        scan_targets.append((DEFAULT_CLAUDE_SESSION_DIR, SessionFormat.CLAUDE_CODE))
    elif session_format == SessionFormat.ANTIGRAVITY:
        scan_targets.append((DEFAULT_ANTIGRAVITY_SESSION_DIR, SessionFormat.ANTIGRAVITY))
...
def _session_core_format(session: Session) -> SessionFormat:
    ...
    if session.format == SessionFormatEnum.CLAUDE_CODE:
        ...
        return SessionFormat.OPENCODE
    elif session.format == SessionFormatEnum.ANTIGRAVITY:
        return SessionFormat.ANTIGRAVITY
    return SessionFormat.CODEX
...
def preview_session(file_path: str, mock_response: str = MOCK_RESPONSE,
    ...
    for idx, line in enumerate(parsed_lines):
        ...
        # Antigravity Desktop：transcript.jsonl 事件格式
        if session_format == SessionFormat.ANTIGRAVITY or line.get('source') in ('MODEL', 'USER_EXPLICIT', 'SYSTEM'):
            ...
            elif source == 'MODEL' and event_type == 'PLANNER_RESPONSE':
                role = 'assistant'
                content = strategy.extract_text_content(line)
            elif line.get('role') in ('user', 'assistant'):
                # 兼容少数导出的 role/content 结构
                role = line.get('role')
                content = strategy.extract_text_content(line)

        # Claude Code / OpenCode 格式
        elif line_type == 'human':
            role = 'user'
...
async def _build_ctf_status_response() -> CTFStatusResponse:
    ...
    return CTFStatusResponse(
        ...
        antigravity_profile_installed=status.antigravity_profile_installed,
        antigravity_profile_workspace_exists=status.antigravity_profile_workspace_exists,
        antigravity_profile_prompt_exists=status.antigravity_profile_prompt_exists,
        antigravity_profile_workspace_path=status.antigravity_profile_workspace_path,
        antigravity_profile_config_path=status.antigravity_profile_config_path,
        antigravity_profile_prompt_path=status.antigravity_profile_prompt_path,
        antigravity_profile_launcher_path=status.antigravity_profile_launcher_path,
        antigravity_global_installed=status.antigravity_global_installed,
        antigravity_global_config_exists=status.antigravity_global_config_exists,
        antigravity_global_config_path=status.antigravity_global_config_path,
        antigravity_global_prompt_path=status.antigravity_global_prompt_path,
        antigravity_global_injection_mode=status.antigravity_global_injection_mode,
        antigravity_global_workspace_count=status.antigravity_global_workspace_count,
    )
...
@router.post("/ctf/antigravity/install", response_model=CTFInstallResponse)
async def install_antigravity_ctf_config(body: Optional[CTFInstallRequest] = None):
    """安装 Antigravity Desktop Profile CTF 配置"""
    from codex_session_patcher.ctf_config import AntigravityCTFInstaller
    installer = AntigravityCTFInstaller()
    settings_data = _load_raw_config()
    custom_prompt = _get_saved_antigravity_prompt_from_config(settings_data)
    ...
    return CTFInstallResponse(
        success=success,
        message=message,
        profile_command=status.antigravity_profile_config_path or "",
        activation_command="在当前 Antigravity 项目中新建对话",
        status=status,
    )


@router.post("/ctf/antigravity/uninstall", response_model=CTFInstallResponse)
async def uninstall_antigravity_ctf_config():
    """卸载 Antigravity Desktop Profile CTF 配置"""
    from codex_session_patcher.ctf_config import AntigravityCTFInstaller
    installer = AntigravityCTFInstaller()
...
@router.post("/ctf/antigravity/global/install", response_model=CTFInstallResponse)
async def install_antigravity_global_ctf_config(body: Optional[CTFInstallRequest] = None):
    """安装 Antigravity Desktop 全局 CTF 配置"""
    from codex_session_patcher.ctf_config import AntigravityCTFInstaller
    installer = AntigravityCTFInstaller()
    settings_data = _load_raw_config()
    custom_prompt = _get_saved_antigravity_prompt_from_config(settings_data)
    ...
    return CTFInstallResponse(
        ...
        message=message,
        profile_command="",
        activation_command="直接新建 Antigravity Desktop 会话",
        status=await _build_ctf_status_response(),
    )


@router.post("/ctf/antigravity/global/uninstall", response_model=CTFInstallResponse)
async def uninstall_antigravity_global_ctf_config():
    """卸载 Antigravity Desktop 全局 CTF 配置"""
    from codex_session_patcher.ctf_config import AntigravityCTFInstaller
    installer = AntigravityCTFInstaller()
...
def _get_saved_antigravity_prompt_from_config(config: dict) -> str | None:
    """读取 Antigravity 保存提示词；没有时兼容旧 Codex 槽位并转换语境。"""
    saved = config.get('ctf_prompts', {}).get('antigravity', {}).get('prompt')
    if saved:
        return saved
    legacy_saved = config.get('ctf_prompts', {}).get('codex', {}).get('prompt')
    if legacy_saved:
        return _adapt_legacy_codex_prompt_for_antigravity(legacy_saved)
    return None


_CTF_PROMPT_PATHS = {
    'antigravity': default_antigravity_prompt_path(),
...
def _get_ctf_prompt_path(tool: str) -> str | None:
    ...
    if tool == 'antigravity':
        ...
        if status.antigravity_profile_installed and status.antigravity_profile_prompt_path:
            return status.antigravity_profile_prompt_path
        if status.antigravity_global_installed and status.antigravity_global_prompt_path:
            return status.antigravity_global_prompt_path
        return _CTF_PROMPT_PATHS['antigravity']
    if tool != 'codex':
...
def _get_codex_prompt_path_for_file(filename: str) -> str:
    ...
    return os.path.join(expand_user_path("~/.codex/prompts"), os.path.basename(filename))
...
def _antigravity_ctf_config_active() -> bool:
    from codex_session_patcher.ctf_config import check_ctf_status
    status = check_ctf_status()
    return status.antigravity_profile_installed or status.antigravity_global_installed


def _sync_antigravity_prompt_config(prompt: str):
    """同步已启用 Antigravity Profile/全局模式中的提示词文件。"""
    from codex_session_patcher.ctf_config import AntigravityCTFInstaller, check_ctf_status
    installer = AntigravityCTFInstaller()
    status = check_ctf_status()
    if status.antigravity_profile_installed:
        installer.update_profile_prompt(prompt)
    if status.antigravity_global_installed:
        installer.update_global_prompt(prompt)


def _extract_antigravity_rule_prompt(content: str) -> str:
    """从 Antigravity .agent/rules/*.md 中提取真正的 prompt 正文。"""
    from codex_session_patcher.ctf_config.status import ANTIGRAVITY_MARKER
    if ANTIGRAVITY_MARKER not in content:
        return content
    lines = content.splitlines()
    marker_index = None
    for index, line in enumerate(lines):
        if ANTIGRAVITY_MARKER in line:
            marker_index = index
            break
    if marker_index is None:
        return content
    body = "\n".join(lines[marker_index + 1:]).lstrip("\n").rstrip()
...
def _read_ctf_prompt_for_tool(tool: str) -> str | None:
    ...
    if path and os.path.exists(path):
        with open(path, 'r', encoding='utf-8') as f:
            content = f.read()
        if tool == 'antigravity':
            return _extract_antigravity_rule_prompt(content)
        return content
    # 其次读用户保存到配置的自定义提示词
    config = _load_raw_config()
    saved = config.get('ctf_prompts', {}).get(tool, {}).get('prompt')
    if saved is None and tool == 'antigravity':
        legacy_saved = config.get('ctf_prompts', {}).get('codex', {}).get('prompt')
        if legacy_saved:
            saved = _adapt_legacy_codex_prompt_for_antigravity(legacy_saved)
    ...
    if is_installed:
        try:
            with open(prompt_path, 'r', encoding='utf-8') as f:
                prompt = f.read()
            if tool == 'antigravity':
                prompt = _extract_antigravity_rule_prompt(prompt)
            return {
                "prompt": prompt,
                "is_installed": True,
                "is_default": prompt.strip() == default_prompt.strip(),
...
async def save_ctf_prompt(tool: str, body: dict):
    ...
    elif tool == 'antigravity' and _antigravity_ctf_config_active():
        _sync_antigravity_prompt_config(prompt)
        should_write_installed = False
...
async def reset_ctf_prompt(tool: str):
    ...
    if tool == 'codex' and _codex_ctf_config_active():
        ...
        _sync_antigravity_prompt_config(default_prompt)
        should_write_installed = False
...
def _adapt_legacy_codex_prompt_for_antigravity(prompt: str) -> str:
    """把历史 Codex 模板内容迁移成 Antigravity 语境。"""
    from codex_session_patcher.ctf_config.templates import adapt_codex_prompt_to_antigravity
    return adapt_codex_prompt_to_antigravity(prompt or "")


def _ensure_antigravity_legacy_templates_migrated(config: dict) -> bool:
    """把旧配置里 ctf_templates.codex 的用户模板复制到 antigravity 槽位。

    这解决从旧版 UI 迁移后，Antigravity 页只剩内置模板、用户自定义模板“消失”的问题。
    只复制缺失名称，不删除/修改原 Codex 模板，最多补到 MAX_TEMPLATES 个 Antigravity 用户模板。
    """
    ...
    antigravity_templates = all_templates.setdefault('antigravity', [])
    existing_names = {tpl.get('name') for tpl in antigravity_templates}
    changed = False
    ...
    for tpl in codex_templates:
        ...
        if not name or name in existing_names:
            continue
        if len(antigravity_templates) >= MAX_TEMPLATES:
            break
        antigravity_templates.append({
            "name": name,
            "prompt": _adapt_legacy_codex_prompt_for_antigravity(tpl.get('prompt', '')),
            "migrated_from": "codex",
        })
    ...
    if changed:
        all_templates['antigravity'] = antigravity_templates
    return changed


def _get_user_templates(config: dict, tool: str) -> list[dict]:
    """读取用户模板；Antigravity 会先兼容迁移旧 Codex 用户模板。"""
    if tool == 'antigravity' and _ensure_antigravity_legacy_templates_migrated(config):
        _save_raw_config(config)
