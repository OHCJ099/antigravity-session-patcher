    ...
    <n-space vertical size="large">
      ...
      <n-card :title="$t('enhance.ctfMode')" size="small">
        <n-tabs type="line" animated>
          ...
          <!-- ── Antigravity Desktop ── -->
          <n-tab-pane name="antigravity" display-directive="show">
            <template #tab>
              <span class="tab-label">
                <span class="status-dot" :class="{
                  'dot-success': ctfStore.status?.antigravity_profile_installed || ctfStore.status?.antigravity_global_installed,
                  'dot-warning': !ctfStore.status?.antigravity_profile_installed && ctfStore.status?.antigravity_global_installed,
                }"></span>
                Antigravity Desktop
              </span>
            </template>
            ...
            <n-space vertical size="large" style="padding-top: 4px">
              ...
              <div class="mode-section">
                ...
                <n-spin :show="ctfStore.prompts.antigravity.loading" style="margin-top: 8px">
                  <div class="template-row">
                    <n-select v-model:value="antigravitySelectedTemplate" size="small" :placeholder="ctfStore.templates.antigravity.length === 0 ? $t('enhance.noTemplates') : $t('enhance.selectTemplate')" :options="templateOptions('antigravity')" :disabled="ctfStore.templates.antigravity.length === 0" :render-label="(option) => renderTemplateLabel(option, 'antigravity')" clearable style="flex: 1" @update:value="(v) => { if (v) applyTemplate('antigravity', v) }" />
                    <n-button size="small" :disabled="ctfStore.templates.antigravity.length >= MAX_TEMPLATES" @click="openSaveTemplate('antigravity')">+ {{ $t('enhance.saveAsTemplate') }}</n-button>
                  </div>
                  <n-input v-model:value="antigravityPromptText" type="textarea" :rows="8" style="font-family: monospace; font-size: 12px" />
                  <n-space style="margin-top: 8px" align="center">
                    <n-button size="small" :disabled="ctfStore.prompts.antigravity.is_default" @click="handleResetPrompt('antigravity')">{{ $t('enhance.restoreDefault') }}</n-button>
                    <n-button size="small" type="primary" @click="handleSavePrompt('antigravity', antigravityPromptText)">{{ $t('common.save') }}</n-button>
                  </n-space>
                </n-spin>
              </div>
              ...
              <div class="mode-section">
                <div class="mode-header">
                  <n-text strong>{{ $t('enhance.antigravityInjectionMode') }}</n-text>
                </div>
                <n-space vertical size="small" style="margin-top: 4px">
                  <n-tag size="small" type="info" :bordered="false">always_on_rule</n-tag>
                  <n-text depth="3" style="font-size: 13px">{{ $t('enhance.antigravityInjectionDesc') }}</n-text>
                </n-space>
              </div>
              ...
              <div class="mode-section">
                <div class="mode-header">
                  ...
                  <n-tag :type="ctfStore.status?.antigravity_profile_installed ? 'success' : 'default'" size="small" :bordered="false">
                    {{ ctfStore.status?.antigravity_profile_installed ? $t('common.enabled') : $t('common.disabled') }}
                  </n-tag>
                </div>
                <n-text depth="3" style="font-size: 13px; line-height: 1.6">{{ $t('enhance.antigravityProfileDesc') }}</n-text>
                <div style="margin-top: 8px">
                  <n-button v-if="!ctfStore.status?.antigravity_profile_installed" type="primary" size="small" :loading="ctfStore.antigravityInstallLoading" @click="handleAntigravityInstall">{{ $t('enhance.enable') }}</n-button>
                  <n-button v-else type="warning" size="small" :loading="ctfStore.antigravityInstallLoading" @click="handleAntigravityUninstall">{{ $t('enhance.disable') }}</n-button>
                </div>
                <p v-if="ctfStore.status?.antigravity_profile_installed" class="command-inline-hint">
                  {{ $t('enhance.profileConfigPath') }}: <code>{{ ctfStore.status?.antigravity_profile_config_path }}</code>
                </p>
              </div>
              ...
              <div class="mode-section">
                <div class="mode-header">
                  ...
                  <n-tag :type="ctfStore.status?.antigravity_global_installed ? 'warning' : 'default'" size="small" :bordered="false">
                    {{ ctfStore.status?.antigravity_global_installed ? $t('common.enabled') : $t('common.disabled') }}
                  </n-tag>
                </div>
                <n-text depth="3" style="font-size: 13px; line-height: 1.6">{{ $t('enhance.antigravityGlobalDesc') }}</n-text>
                <div style="margin-top: 8px">
                  <n-button v-if="!ctfStore.status?.antigravity_global_installed" type="primary" size="small" :loading="ctfStore.antigravityGlobalInstallLoading" @click="handleAntigravityInstallGlobal">{{ $t('enhance.enableGlobal') }}</n-button>
                  <n-button v-else type="warning" size="small" :loading="ctfStore.antigravityGlobalInstallLoading" @click="handleAntigravityUninstallGlobal">{{ $t('enhance.disableGlobal') }}</n-button>
                </div>
                <n-alert v-if="ctfStore.status?.antigravity_global_installed" type="warning" :bordered="false" style="margin-top: 8px">{{ $t('enhance.antigravityGlobalWarning') }}</n-alert>
                <p v-if="ctfStore.status?.antigravity_global_installed" class="command-inline-hint">
                  manifest: <code>{{ ctfStore.status?.antigravity_global_config_path }}</code>
                  <span v-if="ctfStore.status?.antigravity_global_workspace_count"> · {{ ctfStore.status?.antigravity_global_workspace_count }} workspaces</span>
                </p>
              </div>
            </n-space>
          </n-tab-pane>
        ...
        <n-tabs type="segment" size="small">
          <n-tab-pane name="antigravity" tab="Antigravity">
            <n-steps vertical :current="0" size="small" style="margin-top: 12px">
              ...
              <n-step :title="$t('help.workflowCtfSteps[1]')" :description="$t('enhance.antigravityWorkflowStart')" />
...
const anyCtfEnabled = computed(() =>
  ctfStore.status?.antigravity_profile_installed ||
  ctfStore.status?.antigravity_global_installed ||
...
const antigravityPromptText = ref('')
const antigravitySelectedTemplate = ref(null)
...
onMounted(async () => {
  await Promise.all([
    ...
    ctfStore.fetchPrompt('antigravity'),
    ctfStore.fetchPrompt('claude_code'),
    ctfStore.fetchPrompt('opencode'),
    ctfStore.fetchTemplates('antigravity'),
    ctfStore.fetchTemplates('claude_code'),
    ctfStore.fetchTemplates('opencode'),
  ])
  antigravityPromptText.value = ctfStore.prompts.antigravity.prompt
  ...
  for (const tool of ['antigravity', 'claude_code', 'opencode']) {
...
async function handleAntigravityInstall() {
  const result = await ctfStore.installAntigravity()
  message[result.success ? 'success' : 'error'](result.message)
}

async function handleAntigravityUninstall() {
  dialog.warning({
    title: t('common.confirm'),
    content: t('enhance.confirmDisableAntigravity'),
    positiveText: t('common.confirm'),
    negativeText: t('common.cancel'),
    onPositiveClick: async () => {
      const result = await ctfStore.uninstallAntigravity()
      message[result.success ? 'success' : 'error'](result.message)
    }
  })
}

async function handleAntigravityInstallGlobal() {
  const result = await ctfStore.installAntigravityGlobal()
  message[result.success ? 'success' : 'error'](result.message)
}

async function handleAntigravityUninstallGlobal() {
  dialog.warning({
    title: t('common.confirm'),
    content: t('enhance.confirmDisableAntigravityGlobal'),
    positiveText: t('common.confirm'),
    negativeText: t('common.cancel'),
    onPositiveClick: async () => {
      const result = await ctfStore.uninstallAntigravityGlobal()
      message[result.success ? 'success' : 'error'](result.message)
    }
  })
}
