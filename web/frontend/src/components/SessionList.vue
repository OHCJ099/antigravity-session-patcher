<template>
  <div class="session-list">
    ...
    <!-- 格式 Tab（多格式时显示） -->
    <div v-if="settingsStore.antigravityEnabled || settingsStore.claudeCodeEnabled || settingsStore.opencodeEnabled" class="format-tabs">
      <button
        v-if="settingsStore.antigravityEnabled"
        class="format-tab"
        :class="{ active: sessionStore.activeTab === 'antigravity' }"
        @click="sessionStore.setActiveTab('antigravity')"
      >
        {{ $t('session.format_antigravity') }}
        <span class="tab-count">{{ sessionStore.antigravitySessions.length }}</span>
      </button>
      <button
        class="format-tab"
...
// 关闭 Antigravity 支持时，强制切回 Codex
watch(() => settingsStore.antigravityEnabled, (val) => {
  if (!val && sessionStore.activeTab === 'antigravity') {
    sessionStore.setActiveTab('codex')
  }
})
...
const visibleSessions = computed(() => {
  ...
  if (!settingsStore.antigravityEnabled && sessionStore.activeTab === 'antigravity') {
    return sessionStore.codexSessions
  }
  if (!settingsStore.opencodeEnabled && sessionStore.activeTab === 'opencode') {
    return sessionStore.codexSessions
  }
