...
export const useSessionStore = defineStore('session', () => {
  ...
  const activeTab = ref('antigravity') // 'antigravity' | 'codex' | 'claude_code' | 'opencode'
  ...
  const antigravitySessions = computed(() => sessions.value.filter(s => s.format === 'antigravity'))

  // 当前 Tab 的会话
  const activeTabSessions = computed(() => {
    if (activeTab.value === 'antigravity') return antigravitySessions.value
  ...
  async function fetchSessions(checkRefusal = true) {
    ...
    try {
      ...
      if (!_tabInitialized) {
        ...
        // Antigravity Desktop fork：本机会话优先
        if (antigravitySessions.value.length > 0) {
          activeTab.value = 'antigravity'
        } else if (codexSessions.value.length > 0) {
